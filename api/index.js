/**
 * الربح عربي — Server-Side API v2.0
 * ✅ Session-bound ad_nonce  (nonce مخزّن في sessions — لا يُرسل للعميل أبداً)
 * ✅ Multi-account detection (IP + fingerprint clustering)
 * ✅ Nonce مخفي عن المستخدم  (لا يظهر في Network tab)
...
const CFG = {
  ...
  ADS_DAILY_LIMIT:       10,
  AD_NONCE_TTL_SEC:      120,   // 2 دقيقة صلاحية ad_nonce
  ...
  MULTI_ACCT: {
    MAX_PER_IP:          3,   // حد الحسابات لكل IP
    MAX_PER_FP:          1,   // حد الحسابات لكل fingerprint (1 = جهاز واحد فقط)
    NEW_ACCOUNT_DAYS:    3,   // الحساب الجديد (أيام)
    IP_WHITELIST:        new Set([
      // أضف ip_hash لشبكات موثوقة
      // مثال: 'abc123def456...'
    ]),
  },
};
...
async function bootstrap() {
  ...
  try {
    ...
    // ── sessions — يحتوي الآن على ad_nonce مدمج (مخفي عن العميل) ──
    ...
    // ── Migrations — إضافة أعمدة الـ ad_nonce لجدول sessions القديم ──
    const migrations = [
    ...
    console.log('[DB] bootstrap OK v2.0');
  } catch(e) {
...
setInterval(async () => {
  try {
    ...
    // تنظيف ad_nonce المنتهية من الجلسات الحية
    await sql(`UPDATE sessions SET ad_nonce=NULL,ad_nonce_exp=NULL WHERE ad_nonce_exp < NOW()`);
...
function hashIp(ip) { return sha256((ip || '') + (process.env.IP_SALT || 'rebh_ip_salt_2025')).slice(0, 32); }
function hashFp(fp) {
  try { return sha256(typeof fp === 'string' ? fp : JSON.stringify(fp)).slice(0, 40); }
...
async function validateSession(sid) {
  ...
  const r = await sql(
    `SELECT s.*,u.is_banned,u.is_shadow_banned,u.cluster_ban
     ...
     WHERE s.id=$1 AND s.is_revoked=FALSE AND s.expires_at>NOW() LIMIT 1`,
    [sid]
  );
...
async function createSessionRow(userId, fpHash, ipHash) {
  ...
  await sql(
    `INSERT INTO sessions(id,user_id,fingerprint_hash,ip_hash,expires_at) VALUES($1,$2,$3,$4,$5)`,
...
async function consumeNonce(nonce, userId) {
  ...
  const r = await sql(
    `UPDATE nonces SET used=TRUE WHERE nonce=$1 AND user_id=$2 AND used=FALSE AND expires_at>NOW() RETURNING nonce`,
    [nonce, userId]
  );
  return r.length > 0;
}

// ── Ad Nonce: مخزّن في sessions — لا يُرسل للعميل أبداً ────────────
// السيرفر يُنشئه عند start_ad ويُلغيه بعد watch_ad مباشرة
async function issueAdNonce(sessionId) {
  const nonce = genToken(32);
  const exp   = new Date(Date.now() + CFG.AD_NONCE_TTL_SEC * 1000).toISOString();
  await sql(
    `UPDATE sessions SET ad_nonce=$2, ad_nonce_exp=$3 WHERE id=$1`,
    [sessionId, nonce, exp]
  );
  // ✅ لا نُعيد nonce للعميل — السيرفر يحتفظ به فقط
  return true;
}

async function consumeAdNonce(sessionId) {
  if (!sessionId) return false;
  const rows = await sql(
    `SELECT ad_nonce, ad_nonce_exp FROM sessions WHERE id=$1 AND is_revoked=FALSE`,
    [sessionId]
  );
  ...
  if (!ad_nonce_exp || new Date(ad_nonce_exp) < new Date()) {
    await sql(`UPDATE sessions SET ad_nonce=NULL,ad_nonce_exp=NULL WHERE id=$1`, [sessionId]);
    return false;
  }
  // ✅ موجود وصالح — احذفه فوراً (one-time use)
  await sql(`UPDATE sessions SET ad_nonce=NULL,ad_nonce_exp=NULL WHERE id=$1`, [sessionId]);
  return true;
}
  ...
  const r = await sql(
    `INSERT INTO users(tg_id,tg_username,tg_first_name,tg_last_name,tg_language_code,tg_is_premium,ip_hash,fp_hash,last_ip_hash)
     ...
     ON CONFLICT(tg_id) DO UPDATE SET
       ...
       tg_last_name=EXCLUDED.tg_last_name, tg_language_code=EXCLUDED.tg_language_code,
       tg_is_premium=EXCLUDED.tg_is_premium, last_ip_hash=EXCLUDED.last_ip_hash,
       updated_at=NOW()
     RETURNING *`,
    [tgUser.id, tgUser.username || null, tgUser.first_name || null,
     tgUser.last_name || null, tgUser.language_code || 'ar', !!tgUser.is_premium,
     ipHash, fpHash, ipHash]
  );
  return r[0];
}
...
async function addRisk(userId, type, ipHash, fpHash, delta) {
  try {
    await sql(
      `INSERT INTO risk_events(user_id,event_type,ip_hash,fp_hash,score_delta) VALUES($1,$2,$3,$4,$5)`,
...
// ── Multi-Account Detection ───────────────────────────────────────
// يُشغَّل عند create_session — يفحص IP + fingerprint مقابل الحسابات الأخرى
async function checkMultiAccount(userId, ipHash, fpHash) {
  const cfg = CFG.MULTI_ACCT;

  // ✅ IP في whitelist؟ تجاوز الفحص
  if (cfg.IP_WHITELIST.has(ipHash)) return;

  // ── فحص 1: عدد حسابات نشطة بنفس IP ────────────────────────────
  const ipAccounts = await sql(
    `SELECT COUNT(DISTINCT u.id) AS cnt
     FROM users u
     WHERE u.last_ip_hash=$1
       AND u.id != $2
       AND u.is_banned = FALSE`,
    [ipHash, userId]
  );
  const ipCount = parseInt(ipAccounts[0]?.cnt) || 0;
  if (ipCount >= cfg.MAX_PER_IP) {
    await addRisk(userId, 'multi_account_ip', ipHash, fpHash, 20);
    console.warn(`[MULTI] IP cluster detected: user=${userId} shared_ip_count=${ipCount}`);
  }

  // ── فحص 2: fingerprint مطابقة لحساب آخر (تلاعب واضح) ──────────
  ...
  const fpCount = parseInt(fpAccounts[0]?.cnt) || 0;
  if (fpCount >= cfg.MAX_PER_FP) {
    // fingerprint مكرر = حساب متعدد على نفس الجهاز — حظر فوري
    await addRisk(userId, 'multi_account_fp', ipHash, fpHash, 50);
    // حظر تلقائي للحسابات الأخرى بنفس الـ fingerprint
    await sql(
      `UPDATE users SET cluster_ban=TRUE WHERE fp_hash=$1 AND id!=$2`,
      [fpHash, userId]
    );
    console.warn(`[MULTI] FP cluster detected — shadow ban applied: user=${userId}`);
  }

  // ── فحص 3: حسابات محظورة على نفس IP (cluster IP ban) ──────────
  const bannedOnIp = await sql(
    `SELECT COUNT(*) AS cnt FROM users WHERE last_ip_hash=$1 AND is_banned=TRUE AND id!=$2`,
    [ipHash, userId]
  );
  const bannedCount = parseInt(bannedOnIp[0]?.cnt) || 0;
  if (bannedCount > 2) {
...
async function handleCreateSession(body, ipHash, fpHash) {
  ...
  // Referral reward للحسابات الجديدة
  ...
  // إعادة قراءة المستخدم بعد فحص الـ risk
...
// ── start_ad: يُنشئ nonce داخل الجلسة — لا يُرسله للعميل ──────────
async function handleStartAd(userId, sessionId) {
  ...
  await issueAdNonce(sessionId);
  return { ok: true };
}

// ── watch_ad: يتحقق من nonce الجلسة داخلياً — العميل لا يُرسل شيئاً ──
async function handleWatchAd(userId, sessionId, ipHash, fpHash) {
  // ✅ التحقق من nonce مخزّن في الجلسة — بدون أي مدخل من العميل
  const valid = await consumeAdNonce(sessionId);
  if (!valid) {
...
async function handleGetReferrals(userId) {
  ...
  const rR = await sql(
    `SELECT u.tg_first_name,u.tg_username,u.created_at FROM referrals ref JOIN users u ON u.id=ref.referred_id WHERE ref.referrer_id=$1 ORDER BY ref.created_at DESC LIMIT 20`,
    [userId]
  );
  return {
    ok: true,
    ...
    referral_list: rR.map(x => ({ name: x.tg_first_name || x.tg_username || 'مستخدم', ts: new Date(x.created_at).getTime() })),
    tg_id: r.tg_id,
  };
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    switch (type) {
      ...
      // ✅ start_ad: يمرر sessionId — السيرفر يخزّن nonce داخلياً
      case 'start_ad':        result = await handleStartAd(userId, sessionId);                        break;

      // ✅ watch_ad: لا يقبل ad_nonce من العميل — يقرأه من الجلسة مباشرة
      case 'watch_ad':        result = await handleWatchAd(userId, sessionId, ipHash, fpHash);        break;
    ...
    return res.status(500).json({ ok: false, error: 'internal_server_error', ...(IS_DEV && { detail: e.message }) });
  }
};
