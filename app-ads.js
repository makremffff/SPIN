...
export async function watchAd() {
    ...
    if (result.ok) {
        ...
        if (isPartial) {
            ...
            AdNotifQueue.push({ type:'partial', title:`مكافأة جزئية +${pts}`, body: reason });
            pushNotif('gold',`مكافأة جزئية #${result.watchedToday}`,`+${pts} نقطة — ${reason}`);
            // إذا السبب هو عدم ضغط Play → أظهر التذكير في منتصف الشاشة
            if (needsPlay && typeof PlayReminder !== 'undefined') {
                setTimeout(() => PlayReminder.show(), 600);
            }
        } else {
