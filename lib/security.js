...
async function validateSession(sid, ipHash, fpHash) {
  ...
  if (session.fingerprint_hash && fpHash && session.fingerprint_hash !== fpHash) {
    ...
    if (!isNewAccount) {
      await addRisk(session.user_id, 'session_fp_change', ipHash, fpHash, 2,
...
async function consumeNonce(signedNonce, sessionId, userId, fpHash, ipHash, action) {
  ...
  if (n.used) { await addRisk(userId, 'nonce_replay', ipHash, fpHash, 2); return 'replay'; }
  if (new Date(n.expires_at) < new Date()) return 'expired';  // لا risk — انتهاء طبيعي
  if (n.action && n.action !== action) {
    await addRisk(userId, 'nonce_action_mismatch', ipHash, fpHash, 2);
  ...
  if (!constantTimeCompare(clientSig, expectedSig)) {
    await addRisk(userId, 'nonce_signature_invalid', ipHash, fpHash, 3);
    return 'invalid_signature';
  }

  // [4] FP binding — soft check (VPN أو تغيير شبكة لا يستوجب risk عالي)
  if (!constantTimeCompare(n.fp_hash || fpHash, fpHash)) {
    await addRisk(userId, 'nonce_fp_mismatch', ipHash, fpHash, 1);
    return 'fp_mismatch';
  }
...
async function consumeAdNonce(sessionId, signedNonce, userId, ipHash, fpHash) {
  ...
  const parsed = parseSignedNonce(signedNonce);
  if (!parsed) {
    // malformed — signature فاسدة (نادر جداً — 5 نقاط فقط)
    await addRisk(userId, 'reward_ad_nonce_malformed', ipHash, fpHash, 5);
    return 'invalid';
  }
  const { raw: rawNonce, sig: clientSig } = parsed;
  ...
  if (ad_nonce_used) {
    // replay — network retry بسبب timeout (شائع على موبايل) — risk منخفض
    await addRisk(userId, 'reward_ad_nonce_reuse', ipHash, fpHash, 4);
    return 'replay';
  }
  ...
  if (!constantTimeCompare(rawNonce, ad_nonce)) {
    // نونس مختلف — يحدث لما الموبايل ينام أثناء الإعلان ويُعيد تحميل التطبيق
    await addRisk(userId, 'reward_ad_nonce_mismatch', ipHash, fpHash, 5);
    return 'mismatch';
  }
  ...
  if (!constantTimeCompare(clientSig, expectedSig)) {
    // signature مختلفة — قد يكون Vercel cold start / timing issue
    await addRisk(userId, 'reward_ad_signature_invalid', ipHash, fpHash, 5);
    return 'invalid_signature';
  }
...
async function addRisk(userId, type, ipHash, fpHash, delta, meta = {}) {
  try {
    ...
    if (delta > 0) {
      // ── حماية من التراكم: نفس الحدث لا يُحسب أكثر من مرة كل 10 دقائق ──
      ...
      const sameCount = parseInt(recentSame[0]?.cnt) || 0;
      // الأول كامل، الثاني والثالث بـ 50%، بعدها لا شيء
      const effectiveDelta = sameCount <= 1 ? delta
                           : sameCount <= 3 ? Math.ceil(delta / 2)
                           : 0;
      if (effectiveDelta > 0) {
        await sql(`UPDATE users SET risk_score=LEAST(risk_score+$1, 200) WHERE id=$2`, [effectiveDelta, userId]);
      }
      await sql(
...
async function checkMultiAccount(userId, ipHash, fpHash, accountAgeMs) {
  ...
  if ((parseInt(bannedOnIp[0]?.cnt) || 0) > 10) {
    await addRisk(userId, 'cluster_ip_banned', ipHash, fpHash, 5);
  }
