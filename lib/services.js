...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // [7] حساب التذاكر — partial = 50%، full = tickets_per_ad
  const ticketsToAward = isPartialReward
    ? Math.floor(CFG.TICKETS_PER_AD / 2)
    : CFG.TICKETS_PER_AD;
  ...
  // تحديث عداد الإعلانات وآخر مكافأة — بدون إضافة نقاط
  await sql(
    `UPDATE users
  ...
  try {
    ...
    totalTickets = ticketRes?.tickets || 0;
  } catch (_) {}
  ...
  await writeAudit(userId, sessionId, 'reward_ad', 'ok', ipHash, fpHash, {
    watched, tickets_earned: ticketsToAward, partial: isPartialReward,
  });
