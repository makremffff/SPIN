...
// ── نظام الإحالات ─────────────────────────────────────────────────
const { registerReferral, activateReferral, giveAdCommission, getReferrals } = require('../refal');
...
async function handleCreateSession(body, ipHash, fpHash) {
  ...
  try {
    ...
    if (m) {
      await registerReferral(user.id, parseInt(m[1]), tid, tgUser.first_name || tgUser.username);
    }
  } catch (_) {}
  ...
  // referral activation يتم في handleRewardAd عند أول إعلان
...
async function handleClaimAdsgramTask(userId, sessionId, ipHash, fpHash, rawNonce) {
  ...
  try { await giveAdCommission(userId, CFG.USDT_PER_AD); } catch (_) {}
...
// ═══════════════════════════════════════════════════════════════════
// GET REFERRALS — مفوَّض إلى refal.js
// ═══════════════════════════════════════════════════════════════════
async function handleGetReferrals(userId) {
  const res = await getReferrals(userId);
  // map للتوافق مع الواجهة القديمة
  if (!res.ok) return res;
  return {
    ok:                   true,
    total_referrals:      res.total,
    activated_referrals:  res.activated,
    earned_from_refs:     res.earned_usdt,
    pts_per_referral:     res.pts_per_referral,
    referral_list:        res.list,
    tg_id:                res.tg_id,
  };
}
...
async function handleMonetagReward(userId, sessionId, ipHash, fpHash, body) {
  ...
  try { await giveAdCommission(userId, CFG.MONETAG_REWARD * 0.001); } catch (_) {}
