...
async function syncLevel(userId) {
  ...
  if (!r.length) return;
  const oldLevel = parseInt(r[0].level) || 1;
  ...
  if (newLevel > oldLevel && r[0].tg_id) {
    const medals = ['','🥉','🥈','🥇','💎','👑'];
    const medal  = medals[newLevel] || '⭐';
    await sendBotMessage(
      r[0].tg_id,
      `${medal} <b>ترقية جديدة!</b>\n\nتهانينا، وصلت إلى <b>المستوى ${newLevel}</b>!\n\nاستمر في جمع النقاط لفتح مزايا أكثر 🚀`
    );
  }
}
...
async function activatePendingReferral(referredUserId) {
  ...
  // إشعار الـ referrer باسم المُحال
  try {
    const referrerUser = await sql(`SELECT tg_id FROM users WHERE id=$1`, [pendingRef.referrer_id]);
    const referredUser = await sql(`SELECT tg_first_name, tg_username FROM users WHERE id=$1`, [referredUserId]);
    if (referrerUser[0]?.tg_id) {
      const name = referredUser[0]?.tg_first_name || referredUser[0]?.tg_username || 'مستخدم جديد';
      await sendBotMessage(
        referrerUser[0].tg_id,
        `👥 <b>إحالة جديدة!</b>\n\nلقد انضم <b>${name}</b> عبر رابط إحالتك.\n💰 ربحت <b>+${CONFIG.rewards.referral} نقطة</b>\n\nاستمر في الدعوة لتكسب أكثر! 🚀`
      );
    }
  } catch (_) {}
}
...
async function handleSubmitWithdraw(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  // إشعار: طلب السحب قيد المراجعة
  if (nb?.tg_id) {
    await sendBotMessage(
      nb.tg_id,
      `⏳ <b>طلب سحب جديد</b>\n\nتم استلام طلبك وهو الآن <b>قيد المراجعة</b>.\n\n💰 المبلغ: <b>${ton} TON</b>\n📋 رقم الطلب: <b>#${wr[0].id}</b>\n\nسيتم إشعارك فور البت في الطلب.`
    );
  }
...
async function handleAdmin(action, body) {
  ...
  if (action === 'approve_withdrawal') {
    ...
    // جلب بيانات الطلب مع tg_id
    const wrInfo = await sql(
      `SELECT w.ton_amount, w.status, u.tg_id
       FROM withdrawals w JOIN users u ON u.id = w.user_id
       WHERE w.id = $1`, [id]
    );
    if (!wrInfo.length) return { ok: false, error: 'not_found' };
    if (wrInfo[0].status !== 'pending') return { ok: false, error: 'already_resolved', status: wrInfo[0].status };

    // تحديث atomic: فقط إذا لا يزال pending
    const r = await sql(
      `UPDATE withdrawals SET status='completed', tx_hash=$1, reviewed_at=NOW()
    ...
    if (wrInfo[0].tg_id) {
      ...
      await sendBotMessage(
        wrInfo[0].tg_id,
        `✅ <b>تم اعتماد طلب السحب</b>\n\nتهانينا! تم الموافقة على طلب سحبك.\n💰 المبلغ: <b>${parseFloat(wrInfo[0].ton_amount)} TON</b>\n📋 رقم الطلب: <b>#${id}</b>${txLine}\n\nشكراً لاستخدامك التطبيق! 🎉`
      );
    }
    return { ok: true };
  }
  if (action === 'reject_withdrawal') {
    ...
    const wr = await sql(
      `SELECT w.user_id, w.pts, w.status, u.tg_id
       FROM withdrawals w JOIN users u ON u.id = w.user_id
       WHERE w.id = $1`, [id]
    );
    if (!wr.length) return { ok: false, error: 'not_found' };
    if (wr[0].status !== 'pending') return { ok: false, error: 'already_resolved', status: wr[0].status };
    ...
    await sql(
      `UPDATE withdrawals SET status='rejected', reviewed_at=NOW()
