...
// ── Check if referral reward can be given (referred must have > 500 tickets) ──
async function isReferralActive(userId) {
  const ticketRow = (await sql(
    `SELECT COALESCE(SUM(tickets), 0) AS total FROM competition_tickets WHERE user_id=$1`,
    [userId]
  ))[0];
  return (parseInt(ticketRow?.total) || 0) > 500;
}
...
async function activatePendingReferral(referredUserId) {
  ...
  // ── تحقق أن المُحال عنده > 500 تذكرة ──
  const ticketRow = (await sql(
    `SELECT COALESCE(SUM(tickets), 0) AS total FROM competition_tickets WHERE user_id=$1`,
    [referredUserId]
  ))[0];
  const totalTickets = parseInt(ticketRow?.total) || 0;
  if (totalTickets <= 500) return; // لم يصل للحد بعد
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // ── Activate pending referral after watching ad ──────
