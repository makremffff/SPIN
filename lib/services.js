...
// [1] تسجيل إحالة جديدة عند انضمام مستخدم عبر رابط المحيل
async function storeReferral(referrerId, referredUserId, referredName, referrerTgId) {
  ...
  if (!inserted.length) return; // مسجّل مسبقاً
  ...
  // إشعار المحيل بدخول صديقه
  const name = referredName || 'مستخدم جديد';
  await sendBotMessage(
    referrerTgId,
    `🎉 انضم <b>${name}</b> عبر رابط دعوتك!\n\n⏳ سيُحسب رصيدك فور مشاهدته أول إعلان.`
  );
}

// [2] تفعيل الإحالة عند مشاهدة المُحال أول إعلان — يُحسب الرصيد فوراً للمحيل
async function activateReferral(referredUserId) {
  ...
  if (!ref) return; // لا توجد إحالة معلّقة

  // تحديث ذري لمنع التكرار
  const done = await sql(
    `UPDATE referrals
     SET activated=TRUE, activated_at=NOW(), points_given=$1
     WHERE id=$2 AND activated=FALSE
     RETURNING id`,
    [CONFIG.rewards.referral, ref.id]
  );
  if (!done.length) return; // race condition — تمّ بالفعل

  // إضافة المكافأة لرصيد المحيل
  await sql(
    `UPDATE users
     SET usdt_balance=usdt_balance+$1,
         ...
         updated_at=NOW()
     WHERE id=$2`,
    [CONFIG.rewards.referral, ref.referrer_id]
  );

  // جلب بيانات الإشعار
  const [referrerRow, referredRow] = await Promise.all([
    sql(`SELECT tg_id FROM users WHERE id=$1`, [ref.referrer_id]),
    sql(`SELECT tg_first_name, tg_username FROM users WHERE id=$1`, [referredUserId]),
  ]);

  await writeAudit(ref.referrer_id, null, 'referral_activated', 'ok', null, null, {
    referred_id: referredUserId,
    usdt_reward:  CONFIG.rewards.referral,
  });
  console.info(`[REFERRAL] activated — referrer=${ref.referrer_id} referred=${referredUserId} usdt=${CONFIG.rewards.referral}`);

  // إشعار المحيل بأن إحالته أصبحت نشطة
  if (referrerRow[0]?.tg_id) {
    const name = referredRow[0]?.tg_first_name || referredRow[0]?.tg_username || 'إحالتك';
    await sendBotMessage(
      referrerRow[0].tg_id,
      `✅ <b>إحالتك أصبحت نشطة!</b>\n\n👤 <b>${name}</b> شاهد أول إعلان.\n\n💰 تمت إضافة <b>${CONFIG.rewards.referral} USDT</b> إلى رصيدك.`
    );
  }
}
...
async function handleCreateSession(body, ipHash, fpHash) {
    ...
    if (m) {
      ...
      if (refTgId !== tid) {  // prevent self-referral
        ...
        if (rr.length) {
          ...
          const referredName = tgUser.first_name || tgUser.username || 'مستخدم';
          await storeReferral(referrerId, user.id, referredName, refTgId);
        }
      } else {
  ...
  if (userRow?.is_shadow_banned) {
    await writeAudit(user.id, null, 'create_session', 'shadow_banned_allowed', ipHash, fpHash, { tg_id: tid });
    logShadow('create_session_shadow_allowed', user.id, { tg_id: tid });
    // ✅ لا نكسر الجلسة — المستخدم المعلَّق يرى رصيده لكن المكافآت تُحجب بصمت
    // المتابعة ستُضيف _is_shadow_banned: true في الرد النهائي
  }
  ...
  try {
    ...
    if (sessionCount >= 12) {
      // 12+ sessions في 5 دقائق — بوت أو أتمتة واضحة (رُفع من 5 → 12)
      await addRisk(user.id, 'session_farming', ipHash, fpHash, 10, {
  ...
  return {
    ok: true,
    _sid: sessionId,
    _is_shadow_banned: !!(userRow?.is_shadow_banned),
    user: {
...
async function handleLoad(userId) {
  ...
  const wR = await sql(`SELECT pts,ton_amount,address,method,status,created_at FROM withdrawals WHERE user_id=$1 ORDER BY created_at DESC LIMIT 20`, [userId]);
  ...
  return {
    ...
    withdrawals: wR.map(w => ({
      pts: parseInt(w.pts) || 0, usdt: parseFloat(w.ton_amount) || 0, address: w.address,
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // [2] Shadow ban check — silent fake success (المستخدم لا يعلم)
  ...
  if (uR[0]?.is_shadow_banned) {
    ...
    // نُرجع نتيجة وهمية بدون مكافأة — المستخدم لا يرى خطأ
    return {
      ok: true, partial: false, tickets_awarded: 0, total_tickets: 0,
      remaining: 0, watchedToday: 0, earnedToday: 0, points: 0,
      usdt_balance: 0, usdt_earned: 0, all_done: true, cooldown_ms: 90000,
    };
  }
  ...
  // ── Activate referral on first ad watched ────────────────────
  try { await activateReferral(userId); } catch (_) {}
...
async function handleClaimAdsgramTask(userId, sessionId, ipHash, fpHash, rawNonce) {
  ...
  // [4] Shadow ban — silent fake success
  if (u.is_shadow_banned) {
    logShadow('adsgram_claim_blocked', userId, {});
    logRewardBlock('shadow_ban', userId, {});
    return { ok: true, points: 0, reward: 0, state: 'ready', all_done: false };
  }
...
async function handleClaimDailyMission(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  // Check shadow ban — silent fake success
  ...
  if (uCheck?.is_shadow_banned) {
    ...
    return { ok: true, already_claimed: false, reward: 0 };
  }
...
async function handleVerifyChannelTask(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  if (uCheck.is_shadow_banned) {
    ...
    return { ok: true, reward: 0, already_done: false };
  }
...
async function handleVerifyTgTask(userId, rawNonce, sessionId, fpHash, ipHash) {
  ...
  if (uCheck.is_shadow_banned) {
    ...
    return { ok: true, reward: 0, verified: false };
  }
...
async function handleSubmitWithdraw(userId, body, rawNonce, sessionId, fpHash, ipHash) {
  ...
  return {
    ...
    withdraw_id:  wr[0].id,
    usdt_amount:  usdt,
    new_balance:  parseFloat(nb?.usdt_balance) || 0,
    status:       'pending',
