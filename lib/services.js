...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  // Thresholds:
  //   < AD_MIN_DURATION_MS        → rejected (too fast)
  //   30000ms ≤ elapsed ≤ 31999ms → partial reward (50%) — ad shown but no interaction
  //   ≥ 32000ms                   → full reward — user interacted with ad
  ...
  if (clientStartedAt > 0) {
    ...
    if (clientElapsed >= 30000 && clientElapsed <= 31999) {
      isPartialReward = true;
    }
    // clientElapsed >= 32000 → full reward (no flag needed)
  }
