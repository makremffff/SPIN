...
async function activatePendingReferral(referredUserId) {
  ...
  const usdtReward = CONFIG.rewards.referral; // $0.01 USDT لكل إحالة
  ...
  await sql(
    `UPDATE users
     SET usdt_balance=usdt_balance+$1,
         ...
         updated_at=NOW()
     WHERE id=$2`,
    [usdtReward, pendingRef.referrer_id]
  );
  ...
  await writeAudit(pendingRef.referrer_id, null, 'referral_activated', 'ok', null, null, {
    referred_id: referredUserId, usdt_reward: usdtReward,
  });
  console.info(`[REFERRAL] Activated — referrer=${pendingRef.referrer_id} referred=${referredUserId} usdt=${usdtReward}`);
...
async function handleGetReferrals(userId) {
  // ── تفعيل تلقائي لأي إحالة مستحقة عند فتح صفحة الإحالات ────
  try {
    const pending = await sql(
      `SELECT r.id, r.referrer_id, r.referred_id
       ...
       WHERE r.referrer_id = $1
         AND r.activated = FALSE
         AND u.ads_watched_total >= $2`,
      [userId, CONFIG.referral.min_active_ads]
    );
    for (const ref of pending) {
      try {
        const done = await sql(
          `UPDATE referrals SET activated=TRUE, activated_at=NOW()
           WHERE id=$1 AND activated=FALSE RETURNING id`,
          [ref.id]
        );
        if (done.length) {
          await sql(
            `UPDATE users SET usdt_balance=usdt_balance+$1, total_referrals=total_referrals+1, updated_at=NOW() WHERE id=$2`,
