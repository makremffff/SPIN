...
function handleGetConfig() {
  return {
    ...
    session_quality: {
      partial_min_ms:  CONFIG.session_quality.partial_min_ms,
      partial_max_ms:  CONFIG.session_quality.partial_max_ms,
      partial_ratio:   CONFIG.session_quality.partial_ratio,
      full_min_ms:     CONFIG.session_quality.full_min_ms,
      require_play:    CONFIG.session_quality.require_play_click,
    },
    progressive_cooldown: {
      steps_ms: CONFIG.progressive_cooldown.steps_ms,
    },
    adsgram_task: {
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  const sessionDurationMs= parseInt(body?.data?.session_duration_ms) || 0;
  const playClicked      = body?.data?.play_clicked === true;
  const consecutiveCount = parseInt(body?.data?.consecutive_count) || 0;
  ...
  // [4] Progressive cooldown — يتدرج حسب عدد الإعلانات المتتالية
  const coolSteps = CFG.SQ_COOLDOWN_STEPS || [60000, 90000, 120000];
  const stepIdx   = Math.min(consecutiveCount, coolSteps.length - 1);
  const dynamicCooldownMs = coolSteps[stepIdx];
  const coolRow = (await sql(`SELECT ad_last_reward FROM users WHERE id=$1`, [userId]))[0];
  if (coolRow?.ad_last_reward) {
    const elapsed = Date.now() - new Date(coolRow.ad_last_reward).getTime();
    if (elapsed < dynamicCooldownMs) {
      const waitMs = dynamicCooldownMs - elapsed;
      logRewardBlock('cooldown_active', userId, { wait_ms: waitMs, step: stepIdx });
      return { ok: false, error: 'cooldown_active', wait_ms: waitMs, cooldown_step: stepIdx };
    }
  }

  // [5] Client-side min-duration guard
  ...
  if (recentReward.length) {
    ...
    return { ok: false, error: 'cooldown_active', wait_ms: dynamicCooldownMs };
  }

  // ══════════════════════════════════════════════════════════
  // [SESSION QUALITY] — تحديد مستوى المكافأة بناءً على جودة الجلسة
  // partial_min (30s) ≤ duration < full_min (32s) → 50%
  // duration ≥ full_min (32s) + play_clicked       → 100%
  // ══════════════════════════════════════════════════════════
  let rewardRatio  = 1.0;   // default: full
  let sessionGrade = 'full'; // full | partial | minimal

  if (sessionDurationMs > 0) {
    const pMin = CFG.SQ_PARTIAL_MIN_MS || 30000;
    const pMax = CFG.SQ_PARTIAL_MAX_MS || 31000;
    const fMin = CFG.SQ_FULL_MIN_MS    || 32000;
    const requirePlay = CFG.SQ_REQUIRE_PLAY !== false;

    if (sessionDurationMs < pMin) {
      // أقل من 30 ثانية — رُفض في [5] فعلياً لكن كطبقة إضافية
      sessionGrade = 'minimal';
      rewardRatio  = 0;
    } else if (sessionDurationMs <= pMax) {
      // 30-31 ثانية — مشاركة جزئية
      sessionGrade = 'partial';
      rewardRatio  = CFG.SQ_PARTIAL_RATIO || 0.5;
    } else if (sessionDurationMs >= fMin) {
      // 32 ثانية+ — مشاركة كاملة
      if (requirePlay && !playClicked) {
        // بدون ضغط Play → جزئية
        sessionGrade = 'partial';
        rewardRatio  = CFG.SQ_PARTIAL_RATIO || 0.5;
      } else {
  ...
  const earnedPoints = Math.floor(CFG.POINTS_PER_AD * rewardRatio);
  ...
  const lr = await sql(
    ...
    [userId, earnedPoints, CFG.ADS_DAILY_LIMIT]
  );
  ...
  // حساب الـ cooldown التالي
  const nextConsecutive = consecutiveCount + 1;
  const nextStepIdx     = Math.min(nextConsecutive, coolSteps.length - 1);
  const nextCooldownMs  = coolSteps[nextStepIdx];
  ...
  return {
    ...
    session_grade:   sessionGrade,
    reward_ratio:    rewardRatio,
    is_partial:      sessionGrade === 'partial',
    needs_play:      sessionGrade === 'partial' && sessionDurationMs >= (CFG.SQ_FULL_MIN_MS||32000) && !playClicked,
  };
}
