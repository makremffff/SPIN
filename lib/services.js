...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  const clientEndedAt    = parseInt(body?.data?.ad_ended_at)    || 0;
  const clientDurationMs = parseInt(body?.data?.duration_ms)    || 0;
  const sessionClass     = body?.data?.session_class            || '';
  const networkScore     = parseInt(body?.data?.network_score)  || 0;   // 0–100
  const engagementScore  = parseInt(body?.data?.engagement_score) || 0; // 0–100
  const qualityFlags     = Array.isArray(body?.data?.quality_flags) ? body.data.quality_flags : [];
  ...
  // [5] Client-side timing check — يستخدم duration_ms إذا توفر (أكثر دقة)
  const effectiveElapsed = clientDurationMs > 0
    ? clientDurationMs
    : (clientStartedAt > 0 ? Date.now() - clientStartedAt : 0);

  if (effectiveElapsed > 0 && effectiveElapsed < CFG.AD_MIN_DURATION_MS) {
    await addRisk(userId, 'ad_too_fast_client', ipHash, fpHash, 10);
    logRewardBlock('timing_invalid', userId, { elapsed_ms: effectiveElapsed });
    return { ok: false, error: 'ad_duration_invalid' };
  }

  // [5b] Session quality gate — رفض الجلسات الواضحة الغش
  // 'backgrounded' flag من الكلاينت: نضيف risk لكن لا نرفض فوراً (قد يكون false positive)
  if (qualityFlags.includes('backgrounded')) {
    await addRisk(userId, 'ad_session_backgrounded', ipHash, fpHash, 8);
  }
  if (qualityFlags.includes('suspicious_repeat')) {
    await addRisk(userId, 'ad_suspicious_repeat', ipHash, fpHash, 12);
  }
  // إذا اجتمع backgrounded + implausible_duration → رفض
  if (qualityFlags.includes('backgrounded') && qualityFlags.includes('implausible_duration')) {
    logRewardBlock('session_quality_failed', userId, { flags: qualityFlags });
    return { ok: false, error: 'ad_duration_invalid' };
  }
  ...
  // ── حساب cooldown ديناميكي للرد — يُستخدم إذا الكلاينت لا يملك engine ──
  // consecutive count نحسبه من آخر 10 سجلات في audit_log
  ...
  let dynamicCooldownMs = CFG.AD_COOLDOWN_MS;
  if (recentAds >= 3)      dynamicCooldownMs = Math.max(CFG.AD_COOLDOWN_MS, 120000);
  else if (recentAds >= 2) dynamicCooldownMs = Math.max(CFG.AD_COOLDOWN_MS, 90000);
  else                     dynamicCooldownMs = Math.max(CFG.AD_COOLDOWN_MS, 60000);
  // jitter ±8%
  const jitter = Math.round(dynamicCooldownMs * 0.08 * (Math.random() * 2 - 1));
  dynamicCooldownMs = dynamicCooldownMs + jitter;
