...
async function activatePendingReferral(referredUserId) {
  ...
  // points_given is a legacy BIGINT column — USDT reward tracked in usdt_balance below
  const activated = await sql(
    `UPDATE referrals
     SET activated=TRUE, activated_at=NOW(), points_given=0
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body, rawNonce) {
  ...
  if (watched >= CONFIG.referral.min_active_ads) {
    try { await activatePendingReferral(userId); } catch (refE) { console.warn('[REFERRAL] activation failed:', refE.message); }
  }
