...
async function handleStartAd(userId, sessionId, ipHash, fpHash) {
  ...
  // ✅ الـ nonce يبقى في DB — لا يُرسَل للكليانت
  return { ok: true };
}
...
async function handleRewardAd(userId, sessionId, ipHash, fpHash, body) {
  ...
  // server-side فقط — لا يُقبل أي nonce من الكليانت
  const nonceResult = await consumeAdNonce(sessionId, null, userId, ipHash, fpHash);
  if (nonceResult !== 'ok') {
