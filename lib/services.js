...
async function handleClaimAdsgramTask(userId, sessionId, ipHash, fpHash, rawNonce) {
  ...
  // [6] Give tickets (no points)
  await grantCompTickets(userId, CFG.ADSGRAM_TASK_TICKETS);
  await sql(
  ...
  await writeAudit(userId, sessionId, 'claim_adsgram_task', 'ok', ipHash, fpHash, {
    tickets: CFG.ADSGRAM_TASK_TICKETS, adsgram_state: 'completed',
  });

  logAdsgramState(userId, 'completed', { tickets: CFG.ADSGRAM_TASK_TICKETS });
  ...
  return {
    ...
    earned: CFG.ADSGRAM_TASK_TICKETS,
    reward: CFG.ADSGRAM_TASK_TICKETS,
    tickets_awarded: CFG.ADSGRAM_TASK_TICKETS,
