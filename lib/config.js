...
const CONFIG = Object.freeze({
  ...
  ads: {
    ...
    cooldown_ms:       60 * 1000,   // cooldown أساسي 60s (يتصاعد ديناميكياً في الـ engine)
    min_duration_ms:   30 * 1000,   // 30s الحد الأدنى للاحتساب (من المتطلبات)
  },
  adsgram_task: {
    cooldown_ms:       180 * 1000,
    min_watch_ms:      0,      // 0 — الـ widget يتحكم بوقت المهمة، لا نحتاج حد أدنى
    daily_limit:       20,      // عدد مرات المهمة الإعلانية في اليوم
...
const CFG = {
  ...
  AD_COOLDOWN_MS:        CONFIG.ads.cooldown_ms,        // 60s أساسي
  AD_MIN_DURATION_MS:    CONFIG.ads.min_duration_ms,    // 30s من المتطلبات
