...
async function initApp() {
  ...
  if (res && res.ok) {
    ...
    // 🖐️ الجولة التعليمية (Feature Tour) — تُعرض مرة وحدة فقط بعد شاشة التعليمات
    try {
      initFeatureTour(res.user?.tour_seen === true, res.user?.onboarding_seen === true);
    } catch (err) {
      console.error('[initFeatureTour] failed to initialize', err);
    }
  } else {
...
function initOnboarding(alreadySeen = false) {
  ...
  function finishOnboarding() {
    ...
    // 🖐️ شاشة التعليمات خلصت — لو الجولة التعليمية (Feature Tour) لسا ما اتشافت، ابدأها الآن
    if (tourPending) { tourPending = false; setTimeout(startFeatureTour, 500); }
  }
...
/* ══════════════════════════════════════════════════════
   Feature Tour — 4-step coach-mark walkthrough
   (Invite & Earn → How To Play → Cash Out → Climb Ranks)
   Shown once, after onboarding; suppressed via DB flag (tour_seen).
   Only the tooltip + Skip button capture taps — everything else
   (the veil, the glowing hole, the finger) is purely visual, so
   real nav buttons underneath stay fully clickable.
══════════════════════════════════════════════════════ */
let tourPending = false; // set true if onboarding is still showing when initFeatureTour runs

function initFeatureTour(tourSeen, onboardingSeen) {
  if (tourSeen) return;
  if (onboardingSeen) setTimeout(startFeatureTour, 600);
  else tourPending = true; // finishOnboarding() will start it once the intro closes
}
...
function startFeatureTour() {
  ...
  const veil    = document.getElementById('cm-veil');
  const hole    = document.getElementById('cm-hole');
  const finger  = document.getElementById('cm-finger');
  const tip     = document.getElementById('cm-tip');
  const tipStep = document.getElementById('cm-step');
  const tipHead = document.getElementById('cm-head');
  const tipText = document.getElementById('cm-text');
  const nextBtn = document.getElementById('cm-next');
  const skipBtn = document.getElementById('cm-skip');
  const arrow   = document.getElementById('cm-arrow');
  const dots    = tour.querySelectorAll('.cm-progress i');

  const navBtn = page => document.querySelector(`.nb[data-page="${page}"]`);

  // كل عنصر بالمصفوفة = "لقطة" وحدة من الجولة. حقل dot يحدد أي نقطة تقدّم
  // تضوي فوق (خطوة 1 عندها لقطتين — تأشير على زر الدعوة، وبعدها كرت الربح).
  const PHASES = [
    {
      dot: 0, step: 'STEP 1 OF 4', head: 'Invite & Earn',
      text: 'Tap here to invite friends. You earn USDT automatically from everything they play.',
      target: () => navBtn('referral'), side: 'top', pad: 10,
      awaitNav: 'referral' // بننتظر ضغطة حقيقية على الزر، بعدها ننتقل تلقائياً
    },
    {
    ...
    {
      dot: 1, step: 'STEP 2 OF 4', head: 'How To Play',
      text: 'Tap Game anytime to catch falling coins and dodge bombs before the clock runs out.',
      target: () => navBtn('game'), side: 'top', pad: 10
    },
    {
      dot: 2, step: 'STEP 3 OF 4', head: 'Cash Out Anytime',
      text: 'Tap here to withdraw your USDT.',
      target: () => navBtn('withdraw'), side: 'top', pad: 10,
      awaitNav: 'withdraw'
    },
    {
    ...
    {
      dot: 3, step: 'STEP 4 OF 4', head: 'Climb The Ranks',
      text: 'Tap here to check the leaderboard any time.',
      target: () => navBtn('contest'), side: 'top', pad: 10,
      awaitNav: 'contest'
    },
    {
  ...
  function clearNavHandler() {
    if (navHandler) { navHandler.btn.removeEventListener('click', navHandler.fn); navHandler = null; }
  }
  ...
  function runPhase(n) {
    ...
    const p = PHASES[n];
    const el = p.target();
    if (!el) { advance(); return; } // العنصر مو موجود بهالسياق (مثلاً كارد السحب لسا ما اترندر) — تخطّى بأمان

    const reveal = () => {
      const target = p.target();
      if (!target) { advance(); return; }
      tipStep.textContent = p.step;
      tipHead.textContent = p.head;
      tipText.textContent = p.text;
      dots.forEach((d, di) => d.classList.toggle('on', di === p.dot));
      nextBtn.textContent = p.isLast ? 'Finish' : 'Next';
      nextBtn.style.display = p.awaitNav ? 'none' : 'block'; // خطوات التنقل تتقدّم بضغطة حقيقية على الزر، مو بزر Next
      position(target, p.side, p.pad);
      hole.classList.add('show');
      finger.classList.add('show');
      tip.classList.add('show');

      if (p.awaitNav) {
        const fn = () => setTimeout(() => advance(), 480); // خلي التنقل الحقيقي يخلص أول (animatePage وغيرها)
        navHandler = { btn: el, fn };
  ...
  function advance() {
    i++;
    if (i >= PHASES.length) { endTour(); return; }
    runPhase(i);
  }
  ...
  function endTour() {
    ...
    document.getElementById('cm-progress').classList.remove('show');
    fetchApi({ type: 'markTourSeen' }).catch(err =>
      console.warn('[tour] could not persist seen-state to server', err)
    );
  }
  ...
  document.getElementById('cm-progress').classList.add('show');
  runPhase(0);
}
