// ══════════════════════════════════════════════════════════
// app-core.js — Config · State · Session · API
// ══════════════════════════════════════════════════════════
...
// Backwards-compat aliases (used by other files)
export const USER_STATE        = { get points() { return APP_STATE.balance; }, set points(v) { APP_STATE.balance = v; },
...
export const AD_STATE          = APP_STATE.ads;
export const TASKS_STATE       = APP_STATE.tasks;
export const DAILY_GIFT_STATE  = APP_STATE.dailyGift;
export const NOTIF_LIST_DATA   = APP_STATE.notifications;
export const WITHDRAW_HISTORY  = APP_STATE.withdrawHistory;
export let   _SERVER_CONFIG    = APP_STATE.serverConfig;
...
export async function _fetchAppConfig() {
    try {
...
export function _applyConfigToUI() {
    ...
    document.querySelectorAll('.adsgram-reward-badge,#adsgram-reward-badge').forEach(el => {
        el.textContent = APP_CONFIG.rewards.adsgram_task;
    });
...
// ══════════════════════════════════════════════════════════
// ZERO TRUST — Session · Nonce · Fingerprint
// ══════════════════════════════════════════════════════════
let _sessionId      = null;
let _fpCache        = null;
            ...
            if (result.ok && result._session_token) {
                ...
                if (result.user) {
                    ...
                    if (result.user.streak_day  !== undefined)
                        APP_STATE.dailyGift.dayNumber = Math.max(1, (parseInt(result.user.streak_day) || 0) + 1);
                    const today = new Date().toISOString().slice(0, 10);
                    if (result.user.last_gift_date === today) APP_STATE.dailyGift.claimed = true;
                }
                return true;
            }
            return false;
...
export async function _dbCall(action, data = {}, _externalNonce = null) {
    if (!_sessionId && action !== 'create_session') {
        const ok = await _createSession();
        if (!ok) console.warn('[ZT] No session — fallback for:', action);
    }
    ...
    try {
        ...
        if (!nonce && needsServerNonce && _sessionId) {
            try {
                ...
                if (nd.ok && nd.nonce) nonce = nd.nonce;
            } catch (_) {}
...
export function _applyUserToUI(displayName, photoUrl, userId) {
...
export function _generateFallbackAvatar(name, userId) {
...
export function _applyFallbackUser() {
    const nameEl  = document.getElementById('uc-user-name');
    const photoEl = document.getElementById('uc-user-photo');
...
export function _updateReferralLinkUI() {
    document.querySelectorAll('.referral-link-display').forEach(el => {
        el.textContent = REFERRAL_LINK;
    });
    const inp = document.getElementById('referral-link-input');
    if (inp) inp.value = REFERRAL_LINK;
}

// Expose all public functions to window (needed for inline HTML handlers)
